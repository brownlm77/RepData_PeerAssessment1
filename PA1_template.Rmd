...
dfReplacement <- setNames( aggregate(dfActivity$steps, 
                                     ...
                                     na.rm=TRUE),
                           c("interval", "aveSteps"))
print(head(dfReplacement, 5))


# Create a new dataframe and replace NA values
dfActivityNoNA <- dfActivity
#dfActivityNoNA$oldValue <- dfActivityNoNA$steps  # REMOVE
...
# DELETE
#sprintf("Total steps: %.2f, Number days not NA: %d", sum(dfTotalbyDayNoNA$totalSteps), 
...
A:  Replacing the NA values resulted in 8 days of additional data for the histogram. 
As the expected values were used to replace the missing values, the only bin that changed 
was the central bin (or average bin of histograms).  
